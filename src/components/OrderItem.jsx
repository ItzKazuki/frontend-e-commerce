...
export default function OrderItem({ order }) {
  return (
    <Link to={`${order.id}`} className="w-full mx-auto bg-gray-800 text-white rounded-lg shadow-md p-4">
      <div className="flex justify-between items-center border-b-[1px] pb-1 border-gray-400">
        ...
        <span className="badge badge-success text-white">{order.order_status}</span>
      </div>
      <div className="mt-2">
        <div className="flex items-center">
          ...
          <div className="ml-4">
            ...
            <div className="text-xs text-gray-400">{order.order_items.length} barang</div>
          </div>
        </div>
      </div>
      <div className="flex justify-between items-center">
        ...
        <button className="btn btn-success btn-sm mt-7 text-white">Beli Lagi</button>
      </div>
    </Link>
  );
}
